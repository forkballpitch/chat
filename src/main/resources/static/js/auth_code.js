...
//------------------------------------------------------------------------------
var tail_log_time;
function tail_log_new(file_name, result_id, start_line_num, _request, _unlimit_line) {
    if (file_name == "") return;
    ...
    $.post(g_variable["_POST_URL_"], post_data, function (data) {
        ...
        if (ret.result != null) {
            if (ret.result != "") {
                ...
                if (typeof (_unlimit_line) == "undefined" || _unlimit_line != 1) {
                    if (prev_log.length > 20000) prev_log = "";
                }
                ...
                if (ret.result.indexOf('FINISHED') > -1) {
                    clearTimeout(tail_log_time);
                }
            }

            tail_log_time = setTimeout(function () {
                tail_log_new(file_name, result_id, eval(ret.max_cnt++), _request, _unlimit_line);
            }, 1000);
        }
    });
}
